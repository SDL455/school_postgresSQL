<template>
  <div class="dashboard-page">
    ...
    <div class="stats-grid">
      <div class="stat-card primary">
        ...
        <div class="stat-content">
          <div class="stat-value">{{ stats?.overview?.activeStudents || 0 }}</div>
          <div class="stat-label">ນັກຮຽນທັງໝົດ</div>
          <div class="stat-change positive" v-if="stats?.overview?.maleStudents !== undefined">
      ...
      <div class="stat-card secondary">
        ...
        <div class="stat-content">
          <div class="stat-value">{{ stats?.overview?.activeTeachers || 0 }}</div>
      ...
      <div class="stat-card success">
        ...
        <div class="stat-content">
          <div class="stat-value">{{ stats?.overview?.totalClassrooms || 0 }}</div>
      ...
      <div class="stat-card warning">
        ...
        <div class="stat-content">
          <div class="stat-value">{{ stats?.overview?.totalSubjects || 0 }}</div>
    ...
    <div class="grid gap-6" style="grid-template-columns: repeat(auto-fit, minmax(400px, 1fr));">
      ...
      <div class="card">
        ...
        <div class="card-body">
              ...
              <table class="table">
                ...
                <tbody>
                  <tr v-for="room in stats.classroomStats" :key="room.id">
                    <td>
                      <span class="badge badge-primary">{{ room.roomName }}</span>
                    </td>
          ...
          <div v-else class="text-center p-6 text-muted">
            ບໍ່ມີຂໍ້ມູນ
          </div>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">
            <Icon name="mdi:clipboard-check" size="20" style="margin-right: 8px;" />
            ສະຖິຕິເສັ່ງ/ຕົກ
          </h3>
        </div>
        <div class="card-body">
          <div v-if="stats?.gradeStats" class="flex items-center justify-center gap-8 p-6">
            <div class="text-center">
              <div class="stat-value text-success">{{ stats.gradeStats.passed }}</div>
              <div class="stat-label">ເສັ່ງ</div>
            </div>
            <div class="text-center">
              <div class="stat-value text-error">{{ stats.gradeStats.failed }}</div>
              <div class="stat-label">ຕົກ</div>
            </div>
            <div class="text-center">
              <div class="stat-value text-primary">{{ stats.gradeStats.passRate }}%</div>
    ...
    <div class="card mt-6">
      ...
      <div class="card-body">
        <div v-if="stats?.teachersByDepartment?.length" class="flex flex-wrap gap-4">
          <div 
          ...
          >
            ...
            <div class="stat-content">
              ...
              <div class="stat-label">{{ getDepartmentLabel(dept.department) }}</div>
            </div>
          </div>
        </div>
        <div v-else class="text-center p-6 text-muted">
    ...
    <div v-if="stats?.currentAcademicYear" class="card mt-6">
      <div class="card-header">
        <h3 class="card-title">
          <Icon name="mdi:calendar-school" size="20" style="margin-right: 8px;" />
          ປີຮຽນປັດຈຸບັນ
        </h3>
        <span class="badge badge-success">{{ stats.currentAcademicYear.status }}</span>
      </div>
      <div class="card-body">
        <div class="flex items-center gap-4">
          <Icon name="mdi:calendar" size="24" class="text-primary" />
          <div>
            <div class="font-semibold">{{ stats.currentAcademicYear.yearName }}</div>
            <div class="text-muted text-sm">
              {{ formatDate(stats.currentAcademicYear.startDate) }} - {{ formatDate(stats.currentAcademicYear.endDate) }}
            </div>
          </div>
        </div>
      </div>
