          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="subject in subjects" :key="subject.id">
                ...
                <td>{{ departmentLabels[subject.department] || subject.department }}</td>
                ...
                <td>
                  <div class="flex flex-wrap gap-1">
                    ...
                    <span v-if="!subject.subjectGradeLevels?.length" class="text-muted">-</span>
                  </div>
                </td>
                <td>
                  <span class="badge" :class="subject.status === 'ACTIVE' ? 'badge-success' : 'badge-neutral'">
                    {{ subject.status === 'ACTIVE' ? 'ໃຊ້ງານ' : 'ປິດ' }}
                  </span>
                </td>
                <td>
                  <button class="btn btn-ghost btn-sm" @click="openEditModal(subject)">
                    <Icon name="mdi:pencil" size="16" />
                  </button>
                </td>
              </tr>
              <tr v-if="subjects.length === 0">
                <td colspan="8" class="text-center text-muted py-6">ບໍ່ພົບວິຊາ</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal" style="max-width: 500px;">
        <div class="modal-header">
          <h3 class="modal-title">{{ isEdit ? 'ແກ້ໄຂວິຊາ' : 'ເພີ່ມວິຊາໃໝ່' }}</h3>
        ...
        <div class="modal-body">
          <form @submit.prevent="handleSubmit">
            <div class="form-group">
              ...
              <input v-model="form.subjectCode" type="text" class="form-input" required />
            </div>
            <div class="form-group">
              <label class="form-label">ຊື່ວິຊາ *</label>
              <input v-model="form.subjectName" type="text" class="form-input" required />
            </div>
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="form-group">
                ...
                <input v-model.number="form.credits" type="number" step="0.5" class="form-input" />
              </div>
              <div class="form-group">
                <label class="form-label">ຊົ່ວໂມງ/ອາທິດ</label>
                <input v-model.number="form.hoursPerWeek" type="number" class="form-input" />
              </div>
            </div>
            <div class="form-group">
