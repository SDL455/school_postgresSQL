  ...
  <div>
    ...
    <div class="flex justify-between items-center mb-6">
      <div class="flex items-center gap-4">
        ...
        <select v-model="filterClassroom" class="form-input form-select" style="width: auto;">
          <option value="">ທຸກຫ້ອງ</option>
          <option v-for="room in classrooms" :key="room.id" :value="room.id">
            {{ room.roomName }}
          </option>
        </select>
        <select v-model="filterGender" class="form-input form-select" style="width: auto;">
      ...
      <div class="flex gap-2">
        <button v-if="selectedStudents.length > 0" class="btn btn-secondary" @click="showAssignModal = true">
              ...
              <tr v-for="student in students" :key="student.id">
                ...
                <td>
                  <div class="flex items-center gap-3">
                    <div class="avatar avatar-sm" :class="student.gender === 'MALE' ? 'bg-primary-100 text-primary-700' : 'bg-pink-100 text-pink-700'" style="background: var(--primary-100);">
                      {{ student.firstName.charAt(0) }}
                    </div>
                    <div>
                      <div class="font-medium">{{ student.firstName }} {{ student.lastName }}</div>
                      <div v-if="student.email" class="text-muted text-xs">{{ student.email }}</div>
                    </div>
                  </div>
                </td>
                <td>
                  <span class="badge" :class="student.gender === 'MALE' ? 'badge-primary' : 'badge-secondary'" style="background: var(--secondary-100); color: var(--secondary-700);">
                ...
                <td>
                  <div class="flex gap-1">
                    ...
                    <button class="btn btn-ghost btn-sm text-error" @click="confirmDelete(student)">
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal" style="max-width: 600px;">
        <div class="modal-header">
          <h3 class="modal-title">{{ isEdit ? 'ແກ້ໄຂນັກຮຽນ' : 'ເພີ່ມນັກຮຽນໃໝ່' }}</h3>
          ...
          <form @submit.prevent="handleSubmit">
            <div class="grid grid-cols-2 gap-4">
              <div class="form-group">
                ...
                <input v-model="form.studentCode" type="text" class="form-input" required />
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input v-model="form.guardianName" type="text" class="form-input" />
              </div>
              <div class="form-group">
                <label class="form-label">ເບີໂທຜູ້ປົກຄອງ</label>
                <input v-model="form.guardianPhone" type="tel" class="form-input" />
              </div>
              <div class="form-group">
                <label class="form-label">ຫ້ອງຮຽນ</label>
                <select v-model="form.classroomId" class="form-input form-select">
    ...
    <div v-if="showAssignModal" class="modal-overlay" @click.self="showAssignModal = false">
      <div class="modal" style="max-width: 400px;">
        ...
        <div class="modal-footer">
          <button class="btn btn-outline" @click="showAssignModal = false">ຍົກເລີກ</button>
          <button class="btn btn-primary" :disabled="!assignClassroomId" @click="handleAssignClassroom">
            ແຍກຫ້ອງ
          </button>
        </div>
      </div>
    ...
    <div v-if="showDeleteModal" class="modal-overlay" @click.self="showDeleteModal = false">
      <div class="modal" style="max-width: 400px;">
        ...
        <div class="modal-body">
          <p>ທ່ານແນ່ໃຈບໍ່ວ່າຕ້ອງການລົບນັກຮຽນ <strong>{{ studentToDelete?.firstName }} {{ studentToDelete?.lastName }}</strong>?</p>
...
const { getStudents, createStudent, updateStudent, deleteStudent, getClassrooms, request } = useApi()
